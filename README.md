...
```
* * * * * ubuntu bash /home/ubuntu/apps/bestsongs14/repository/cron.sh fab $DEPLOYMENT_TARGET cron_jobs.test 
```
...
|bestsongs14|amazon-click|`artist` + ' - ' + `title`||||
|bestsongs14|itunes-click|`artist` + `title`||||
|bestsongs14|rdio-click|`artist` + `title`||||
